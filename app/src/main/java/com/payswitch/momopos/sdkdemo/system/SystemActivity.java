    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnReadSN.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(version>=29){
                    Log.i("SN","SERIAL : "+getSN_TAB());
                    tvResult.setText("SN: " + getSN_TAB());
                }
//                else if ((version >= 27)){
//                    Log.i("SN","SN_QT "+getSN_QT());
//                    tvResult.setText("SN: " + getSN_QT());
//                }else {
...
//                    ver = ver.substring(0,index);
                    ...
                    tvResult.setText("code get fail"+i);
            } catch (Exception e) {
                e.printStackTrace();
                tvResult.setText("Exception " + e.toString());
            }
        ...
        Button btnGetSCVersion = (Button)findViewById(R.id.getscver);
        btnGetSCVersion.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                try {
                    String ver = mCore.getSCVersion();
                    tvResult.setText("get SC Version:"+ver);
                } catch (Exception e) {
                    e.printStackTrace();
                    tvResult.setText("Exception " + e.toString());
                }
        ...
        if ("WPOS-TAB".equals(Build.MODEL)){
        ...
        btnGetFirVersion.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    String FirmWareNo = mCore.getFirmWareNumber();
                    if (FirmWareNo == null) {
                        tvResult.setText("");
                    }else {
                        tvResult.setText(FirmWareNo);
                    }
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
...
//                mCore.buzzer();
        ...
        Button paywave_kernel = (Button)findViewById(R.id.paywave_kernel) ;
        paywave_kernel.setVisibility(View.GONE);
        paywave_kernel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
                    //for payWave
                    int res = mCore.SetKernel(0x00,0x48,0x00);
                    if (res == 0) {
                        TradeInfo.getInstance().setKernel(1);
                        tvResult.setText("setting success");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    tvResult.setText("Exception " + e.toString());
        ...
        Button qpboc_kernel = (Button)findViewById(R.id.qpboc_kernel) ;
        qpboc_kernel.setVisibility(View.GONE);
        qpboc_kernel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
                    //for qPBOC
                    int res = mCore.SetKernel(0x00,0x00,0x00);
                    if (res == 0) {
                        TradeInfo.getInstance().setKernel(0);
                        tvResult.setText("setting success");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
...
//                mCore.ledFlash(1,1,1,1,1000,1000,3000);
        ...
        Button Btngetdevicestatus = (Button) findViewById(R.id.get_device_status);
        Btngetdevicestatus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                int ret = 0;
                ...
                try {
                    ret = mCore.getDeviceStatus(outdata, outlen);
                    if (RspCode.OK == ret){
                        tvResult.setText("get device statuc:"+ ByteUtil.bytes2HexString(outdata));
                    }else {
                        tvResult.setText("Get device status error!");
                    }
                } catch (RemoteException e) {
                    e.printStackTrace();
                }
...
//    private String getSN_QT(){
//        String serial = "unknown";
//        try {
//            Class clazz = Class.forName("android.os.Build");
//            Method method = clazz.getMethod("getSerial", null);
//            serial = (String) method.invoke(null);
...
//    }
    ...
    private String getSN_TAB(){
        String serial = "unknown";
        try {// getprop persist.sys.serial
            Class<?> c = Class.forName("android.os.SystemProperties");

            Method get = c.getMethod("get", String.class);

            serial = (String) get.invoke(c, "persist.sys.serial");
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
