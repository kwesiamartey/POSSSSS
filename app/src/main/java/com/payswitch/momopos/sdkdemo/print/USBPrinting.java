...
public class USBPrinting extends AppCompatActivity {
    ...
    private int testPrintLaguage(int result){
        try {
...
//            result = mPrinter.printMultiseriateString(proportionArray,contentArray, 25, Align.LEFT, false, false);
...
//            result = mPrinter.printMultiseriateString(proportionArray,contentArray, 25, Align.LEFT, false, false);
            Log.d(TAG, "testPrintString: result == "+result);
            int[] status = new int[1];
            mPrinter.getPrinterStatus(status);
            Log.e(TAG,"status is hank "+status[0]);
        } catch (RemoteException e){
            e.printStackTrace();
            Log.e(TAG, "testPrintString: 异常 result "+result);
        }
    ...
    private void blueToothPrint(){
        ...
        try {
            ...
            Log.d(TAG, "blueToothPrint: set density low 3");
//            mPrinter.setGrayLevel(3);
//            testPrintImageBase(result);
            // Print text
            mPrinter.setGrayLevel(3);
            testPrintString(-1000);
            // print bar_Code
...
//            //print QR_Code
            ...
            //laguage print
            Log.d(TAG, "blueToothPrint: set density high 1"+result);
            Log.d(TAG, "blueToothPrint: set density high 1");
            mPrinter.setGrayLevel(1);
           result =  testPrintLaguage(-1000);
            //print end reserve height
            Log.d(TAG, "blueToothPrint: hank"+result);
            result = mPrinter.printPaper(50);
            Log.d(TAG, "printPaper result: "+result);

            result = mPrinter.printFinish();
            Log.d(TAG, "printFinish result: "+result);
            ResultInfo msgInfo = new ResultInfo();
            msgInfo.code = result;
            if(result == 0){
