...
public class CardReader extends AppCompatActivity implements View.OnClickListener {
    ...
    private Button mBtnPicc,buttonreadcardicms;
    ...
    private static final int at88sc = 12;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case -1:
                    ...
                    tvcardreadershow.setText(msg.arg1+"---/"+cardType);
                    break;
                case PICC:
                    if (msg.arg1 == 0){
                ...
                case PSAM2:
                    if (msg.arg1 == 0){
                        ...
                        tvcardreadershow.setText("PSAM2 error");
                    }
                    break;
                case AT24:
                ...
                case at88sc:
                    if (msg.arg1 == 0){
                        tvcardreadershow.setText("at88sc102Card read/Write card and Verify success");
                    }else if(msg.arg1==3){
                        tvcardreadershow.setText("time out");
                    }else{
                        tvcardreadershow.setText("at88sc102 error");
                    }
                    break;
                case DESFIRE:
                    if (msg.arg1 == 0){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonreadcardicms = (Button)findViewById(R.id.buttonreadcardicms);
        ...
        buttonM1_quick = (Button)findViewById(R.id.buttonM1_quick);
        ...
        buttonat88sc102 = (Button) findViewById(R.id.buttonat88sc102);
        buttonDesFire = (Button)findViewById(R.id.buttonDesFire);
        mBtnExit = (Button) findViewById(R.id.exit);
        mBtnStop = (Button) findViewById(R.id.btn_stop);
        mBtnNoEMV = (Button) findViewById(R.id.but_no_emv);
        ...
        mBtnArray = new Button[]{mBtnPicc, mBtnIc, mBtnMag, mBtnPsam1, mBtnPsam2,buttonM0,buttonM1,buttonM1_quick,mBtnAT24,button4428,button4442,buttonat88sc102,buttonDesFire,buttonreadcardicms};
        refreshButton(false);
...
//                    | 0x08| 0x11| 0x40| 0x04| 0x14|  0x24|0x34;
            ...
            retvalue = mBankCard.readCard(BankCard.CARD_TYPE_NORMAL, BankCard.CARD_MODE_ICC |BankCard.CARD_MODE_PICC , 60, respdata, resplen, "app1");
//            retvalue = mBankCard.cardReaderDetact(0, 0x02 , cardMode, respdata, resplen, "app1");


            if (retvalue == 0 ) {
                int respdatum = respdata[0];
                String cardType = "";
                switch (respdatum) {
                    case 0X00:
                        cardType = " Magnetic Track Data";
                        break;
                    case 0X01:
                        cardType = "  Read Card failed";
                        break;
                    case 0X02:
                        cardType = " Got Card Magnetic Track Data, Data Encrypt failed";
                        break;
                    case 0X03:
                        cardType = " Timeout";
                        break;
                    case 0X04:
                        cardType = " Cancel Searching Card";
                        break;
                    case 0X05:
                        cardType = "Contact IC card insert";
                        break;
                    case 0X15:
                        cardType = " 4442 Card";
                        break;
                    case 0X25:
                        ...
                        break;
                    case 0X35:
                        cardType = "  AT88SC102 Card detected";
                        break;
                    case 0X45:
                        cardType = "  AT24CXX Card detected.";
                        break;
                    case 0X07:
                        cardType = " Contactless IC Card";
                        break;
                    case 0X27:
                        cardType = " multi-Contactless IC Card are detected";
                        break;
                    case 0X37:
                        cardType = " M1-S50 Card detected";
                        break;
                    case 0X47:
                        cardType = " M1-S70 Card detected";
                        break;
                    case 0X57:
                        cardType = " UL(UltraLight) /NTAG203 Card detected";
                        break;
                    case 0x87:
                        cardType = "  DesFire Card detected";
                        break;
                }
    ...
    public void psam2() {
        ...
        try {
            ...
            psm2apdu = mBankCard.sendAPDU(BankCard.CARD_MODE_PSAM2_APDU, sendapdu, sendapdu.length, resp, resplen);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        Log.v(TAG, "" + resplen[0]);
...
//        String s1 = new String(respdata);
    ...
    private void Card4442Test() {
        ...
        if (respdata[0] == 0x15 ) {
            ...
            try {
                int res =mBankCard.VerifyLogicCardPwd(pwd);
//                Log.d(TAG, "xxxxxxxxxx, res= " + res);
    ...
    private String at88sc102;
    private void Cardbuttonat88sc102Test() {
        ...
        if(respdata[0] == 0x03){
            Message msg = mHandler.obtainMessage(at88sc);
            msg.arg1 = 3;
            mHandler.sendMessage(msg);
            return;
        }else if (respdata[0] == 0x04) {
        ...
        if (retvalue == 0x00) {
            ...
            if (respdata[0] == 0x37||respdata[0] == 0x47) {//0x37:M1-S50卡;0x47:M1-S70卡
                try {
                    ...
                    System.arraycopy(sn,0,snData,0,pes[0]);
                    Log.v(TAG, "m1CardSNFunction, ressnData--->>>"+HEX.bytesToHex(snData));
                    int[] len_readblocknum1= new int[1];
                    byte[] data_readblocknum1 = new byte[20];
                    int[] len_readblocknum2= new int[1];
                    byte[] data_readblocknum2 = new byte[20];
                    int[] len_readblocknum3= new int[1];
                    byte[] data_readblocknum3 = new byte[20];
                    int resKeyAuth = mBankCard.m1CardKeyAuthAndReadBlockData(0x41,0x08,keyData.length,keyData,snData.length,snData,
                            0x08,0x09,0x0A,len_readblocknum1,data_readblocknum1, len_readblocknum2, data_readblocknum2, len_readblocknum3, data_readblocknum3);
                    Log.v(TAG, "m1CardKeyAuthAndReadBlockData, resKeyAuth--->>>"+resKeyAuth);
                    Log.i(TAG, "m1CardKeyAuthAndReadBlockData: len_readblocknum1,data_readblocknum1:"+ByteUtil.bytes2HexString(data_readblocknum1)+
                            "len_readblocknum2, data_readblocknum2:"+ByteUtil.bytes2HexString(data_readblocknum2)+
                            "len_readblocknum3, data_readblocknum3:"+ByteUtil.bytes2HexString(data_readblocknum3));
                    if (resKeyAuth == 0) {
                        stringBuilder.append("M1 m1CardKeyAuthAndReadBlockData success\n");
                        byte[] writeData = new byte[16];
                        writeData[0] = (byte)0x55;
                        writeData[1] = (byte)0x55;
                        writeData[2] = (byte)0x55;
                        writeData[3] = (byte)0x55;
                        writeData[4] = (byte)0xAA;
                        writeData[5] = (byte)0xAA;
                        writeData[6] = (byte)0xAA;
                        writeData[7] = (byte)0xAA;
                        writeData[8] = (byte)0x55;
                        writeData[9] = (byte)0x55;
                        writeData[10] = (byte)0x55;
                        writeData[11] = (byte)0x55;
                        writeData[12] = (byte)0x01;
                        writeData[13] = (byte)0xFE;
                        writeData[14] = (byte)0x01;
                        writeData[15] = (byte)0xFE;
                        byte[] outdata = new byte[20];
                        int[] outdatalen = new int[1];
                        int resWrite = mBankCard.m1CardWriteAndReadBlockData(0x09,writeData.length,writeData,outdata,outdatalen);
                        if (resWrite == 0) {
                            stringBuilder.append("M1 Write success\n");
                            byte[] readData = new byte[20];
                            int[] readLen = new int[1];
                            int resRead = mBankCard.m1CardWriteAndReadBlockData(0x0A,writeData.length,writeData,readData,readLen);
                            if (resRead == 0) {
                                stringBuilder.append("M1 Read success\n");
                                Log.v(TAG, "m1CardReadBlockData, readDataOrl--->>>"+HEX.bytesToHex(readData));
                                byte[] read = new byte[readLen[0]-1];
                                ...
                                int resValue = mBankCard.m1CardValueOperationAndReadBlockData(0x2B,0x0A,1,0x0A,data,dataLen);
                                if (resValue == 0) {
                                    msg.arg1 = 0;
                                    stringBuilder.append("M1 ValueOperation success\n");
                                }else {
...
//            }
    ...
    public void piccCardTestms() {
        ...
        int retvalue = -1;
        try {
            retvalue = mBankCard.readCardms(BankCard.CARD_TYPE_NORMAL, BankCard.CARD_MODE_PICC, 1024, respdata, resplen, "app1");
        } catch (RemoteException e) {
            e.printStackTrace();
        }
